...
const nav=()=>`<a class="skip-link" href="#main">Skip to content</a><div class="nav-wrap"><nav class="nav" aria-label="Main navigation"><a class="logo" href="/" aria-label="Al Najah Dental Lab home">${brand()}</a><div class="nav-bar"><div class="links" id="site-links">${pages.map(navItem).join('')}</div><a class="order" href="${page==='about'?'contact.html#start':'#start'}">Order Now ${icon('arrow')}</a></div><button class="menu" type="button" aria-label="Open navigation" aria-controls="site-links" aria-expanded="false">Menu</button></nav></div>`;
const whatsappIcon=()=>'<img class="icon" src="/assets/icons/whatsapp.png" alt="" aria-hidden="true" width="20" height="20">';
const whatsapp=()=>`<a class="whatsapp-float" href="https://wa.me/97152777075" target="_blank" rel="noopener noreferrer" aria-label="Chat with Al Najah Dental Lab on WhatsApp"><img src="/assets/icons/whatsapp.png" alt="" aria-hidden="true" width="48" height="48"></a>`;
const title=(eye,head,sub)=>`<header class="hero-title"><div class="eyebrow">${eye}</div><h1>${head}</h1><p>${sub}</p></header>`;
const casePanel=()=>`<section class="case-panel" id="start"><div><div class="eyebrow">Start a Case</div><h2>Send us your<br>next case.</h2><p>Partner with Al Najah Dental Lab for precision restorations and end-to-end case support. Submit a brief request, our team responds within one business day.</p><a class="btn" style="background:#fff" href="contact.html">Start a Case</a><div class="contact-actions"><a href="tel:+97125587947"><span class="icon-disc">${icon('phone')}</span><span><small>Landline</small>+97125587947</span></a><a href="https://wa.me/97152777075" target="_blank" rel="noopener noreferrer"><span class="icon-disc">${whatsappIcon()}</span><span><small>WhatsApp</small>+97152777075</span></a><a class="contact-email-action" href="mailto:Info@alnajah-tdl.net"><span class="icon-disc">${icon('email')}</span><span><small>Email</small>Info@alnajah-tdl.net</span></a></div></div><form class="form" data-case-form><div class="eyebrow">[ Case Request ]</div><div class="form-grid"><label class="wide">Your name *<input name="name" autocomplete="name" placeholder="Dr. Full Name" required></label><label>Email *<input name="email" type="email" autocomplete="email" placeholder="you@clinic.com" required></label><label>Clinic<input name="clinic" autocomplete="organization" placeholder="Clinic name"></label><label>Contact number *<input name="phone" type="tel" autocomplete="tel" placeholder="Contact number" required></label><label>Case type<select name="service"><option>Select service</option><option>Fixed Prosthetics</option><option>Implant Solutions</option></select></label></div><button class="btn" type="submit">Submit Case Request ${icon('arrow')}</button><p style="font-size:11px">We will contact you for your case.</p><p class="form-status" role="status"></p></form></section>`;
const footer=()=>`<footer class="footer"><div class="footer-box"><div class="footer-grid"><div><a class="logo" href="/" aria-label="Al Najah Dental Lab home">${brand()}</a><p>The Silent Art Behind Every Confident Smile</p><div class="socials">${['instagram','linkedin','facebook'].map(name=>`<span class="social-icon" role="img" aria-label="${name}">${icon(name)}</span>`).join('')}</div></div><div><h4>Company</h4><a href="about.html">About Us</a><a href="cases.html">Gallery</a><a href="technology.html">Digital Workflow</a><a href="contact.html">Contact</a></div><div><h4>Services</h4><a href="services.html">Fixed Prosthetics</a><a href="services.html">Implant Solutions</a><a href="services.html">Aesthetics</a><a href="technology.html">Digital Dentistry</a></div><div><h4>Stay informed</h4><p>Updates on digital dentistry, new capabilities, and clinical insights.</p><form class="newsletter" data-newsletter><label class="sr-only" for="newsletter-email">Email address</label><input id="newsletter-email" name="email" type="email" required placeholder="Email address" autocomplete="email"><button type="submit">Join</button><p class="form-status" role="status"></p></form></div></div><div class="copyright"><span>© 2026 Al Najah Dental Lab. All rights reserved.</span><span>Privacy　 Terms　 <a href="#top" class="back-top">Back to top ${icon('up')}</a></span></div></div></footer>`;
